...
pub fn solve() {
  ...
  map
  |> get_trailheads
  |> list.map(fn(trailhead) { calc_trailhead_score(map, trailhead) })
...
fn calc_trailhead_score(map: Map, coord: Coord) {
  find_summits(map, coord) |> list.length
}

fn find_summits(map: Map, coord: Coord) {
  ...
  case value {
    ...
    _ -> {
      ...
      |> list.flat_map(fn(neighbor) { find_summits(map, neighbor.1) })
      |> list.unique
    }
  }
}
